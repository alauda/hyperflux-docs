...
- Fixed an issue where the setting the knowledge database name may not work. This fix adds an option
...
- Fixed an issue when asking for disk space information using Agent mode, the server may stuck.
- Fixed an issue when deploying on ACP 4.2 or above, the default node taints are not handled.
- Fixed an deployment error: `kubeVersion: >=1.20.0 which is incompatible with Kubernetes v1.33.7-1`.
- Fixed an issue where the API key for LLM service and rerank service showed in plain text when deploying.
